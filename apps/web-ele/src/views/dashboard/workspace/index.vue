...
async function fetchWeatherByLocation() {
  try {
    // 使用免费的 ip-api.com 获取当前 IP 的经纬度或城市
    const ipRes = await fetch('http://ip-api.com/json/?lang=zh-CN').then((r) =>
      r.json(),
    );

    if (ipRes && ipRes.status === 'success') {
      // 拿到经纬度或城市去查天气
      const res = await getWeatherApi({ lat: ipRes.lat, lon: ipRes.lon });
      weather.value = res;
    }
    loading.value = false;
  } catch (error: any) {
    ...
    if (error instanceof GeolocationPositionError) {
      switch (error.code) {
        case error.PERMISSION_DENIED: {
          console.warn('定位失败：用户拒绝了位置权限或环境非 HTTPS');
          break;
        }
        case error.POSITION_UNAVAILABLE: {
          console.warn('定位失败：无法获取当前位置（没有 GPS/Wi-Fi 信号）');
          break;
        }
        case error.TIMEOUT: {
          console.warn('定位失败：获取位置超时');
          break;
        }
      }
    } else {
      console.warn('定位失败，错误:', error);
    }
